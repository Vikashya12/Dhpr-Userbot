...
from Rudra.helper.PyroHelpers import get_ub_chats
from Rudra.modules.basic.profile import extract_user, extract_user_and_reason
from Rudra.database import gbandb as Rudra
from Rudra.database import gmutedb as Gmute
from Rudra.modules.help import add_command_help
...
async def gban_user(client: Client, message: Message):
    ...
    if (await Rudra.gban_info(user.id)):
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await Rudra.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await Rudra.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await Rudra.gban_info(user_id)):
        try:
